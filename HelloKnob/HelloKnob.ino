...
void lire_historique_depuis_sd() {
    ...
    while(f.available()) {
        ...
        int premiere_virgule = ligne.indexOf(',');
        if (premiere_virgule == -1) continue;
        String chaine_date = ligne.substring(0, premiere_virgule);
        
        int deuxieme_virgule = ligne.indexOf(',', premiere_virgule+1);
        if (deuxieme_virgule == -1) continue;
        String chaine_base = ligne.substring(premiere_virgule+1, deuxieme_virgule);
